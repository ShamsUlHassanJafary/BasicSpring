...
import gms4u.repository.JobRepository;
...
/**
 * REST controller for managing {@link gms4u.domain.Job}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class JobResource {
    ...
    public JobResource(JobRepository jobRepository, GarageAdminService garageAdminService) {
    ...
    public ResponseEntity<Job> createJob(@RequestBody Job job) throws URISyntaxException {
        ...
        if (job.getReference() == null)
            job.setReference(garageAdminService.generateJobReference(job));
