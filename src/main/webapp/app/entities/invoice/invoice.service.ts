import { Injectable } from '@angular/core';
import { HttpClient, HttpResponse } from '@angular/common/http';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';
import * as moment from 'moment';

import { SERVER_API_URL } from 'app/app.constants';
import { createRequestOption } from 'app/shared/util/request-util';
import { IInvoice } from 'app/shared/model/invoice.model';
import { IJob } from 'app/shared/model/job.model';
import { IBooking } from 'app/shared/model/booking.model';

type EntityResponseType = HttpResponse<IInvoice>;
type EntityArrayResponseType = HttpResponse<IInvoice[]>;

@Injectable({ providedIn: 'root' })
export class InvoiceService {
  public resourceUrl = SERVER_API_URL + 'api/invoices';

  public garageResourceUrl = SERVER_API_URL + 'api/garages';

  constructor(protected http: HttpClient) {}

  create(invoice: IInvoice): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(invoice);
    return this.http
      .post<IInvoice>(this.resourceUrl, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }

  update(invoice: IInvoice): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(invoice);
    return this.http
      .put<IInvoice>(this.resourceUrl, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<IInvoice>(`${this.resourceUrl}/${id}`, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IInvoice[]>(this.resourceUrl, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }

  queryInvoiceJobs(bookingId?: number, garageId?: number, req?: any): Observable<HttpResponse<IJob[]>> {
    const options = createRequestOption(req);
    return this.http
      .get<IJob[]>(`${this.garageResourceUrl}/${garageId}/bookings/${bookingId}/jobs`, { params: options, observe: 'response' })
      .pipe(map((res: HttpResponse<IJob[]>) => res));
  }

  queryGarageInvoices(garageId?: number, req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IBooking[]>(`${this.garageResourceUrl}/${garageId}/invoices`, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }

  delete(id: number): Observable<HttpResponse<{}>> {
    return this.http.delete(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  protected convertDateFromClient(invoice: IInvoice): IInvoice {
    const invoiceBooking = invoice.booking;
    const copy: IInvoice = Object.assign({}, invoice, {
      invoiceDate: invoice.invoiceDate && invoice.invoiceDate.isValid() ? invoice.invoiceDate.utc(true).toJSON() : undefined,
      issueDate: invoice.issueDate && invoice.issueDate.isValid() ? invoice.issueDate.utc(true).toJSON() : undefined,
      bookingDate:
        invoiceBooking?.bookingDate && invoiceBooking?.bookingDate.isValid() ? invoiceBooking?.bookingDate.utc(true).toJSON() : undefined,
      bookingTime:
        invoiceBooking?.bookingTime && invoiceBooking?.bookingTime.isValid() ? invoiceBooking?.bookingTime.utc(true).toJSON() : undefined,
    });
    return copy;
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.invoiceDate = res.body.invoiceDate ? moment(res.body.invoiceDate) : undefined;
      res.body.issueDate = res.body.issueDate ? moment(res.body.issueDate) : undefined;
      const invoiceBooking = res.body.booking;
      if (invoiceBooking) {
        invoiceBooking.bookingDate = invoiceBooking.bookingDate ? moment(invoiceBooking.bookingDate) : undefined;
        invoiceBooking.bookingTime = invoiceBooking.bookingTime ? moment(invoiceBooking.bookingTime) : undefined;
      }
    }
    return res;
  }

  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((invoice: IInvoice) => {
        invoice.invoiceDate = invoice.invoiceDate ? moment(invoice.invoiceDate) : undefined;
        invoice.issueDate = invoice.issueDate ? moment(invoice.issueDate) : undefined;
        const invoiceBooking = invoice.booking;
        if (invoiceBooking) {
          invoiceBooking.bookingDate = invoiceBooking.bookingDate ? moment(invoiceBooking.bookingDate) : undefined;
          invoiceBooking.bookingTime = invoiceBooking.bookingTime ? moment(invoiceBooking.bookingTime) : undefined;
        }
      });
    }
    return res;
  }
}
